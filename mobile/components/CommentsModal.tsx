...
const CommentsModal = ({selectedPost,onClose}:CommentsModalProps) => {
  ...
  return (
    <Modal visible={!!selectedPost} animationType='slide' presentationStyle='pageSheet'>
      <View className='flex-row items-center justify-between pt-4 px-4 border-b border-gray-100'>
      ...
      <ScrollView className='flex-1'>
        ...
        '>
          <View className='flex-row'>
            ...
            <View className='flex-1'>
              ...
              {selectedPost.image &&
              <Image source={{uri:selectedPost.image}} className='w-full h-48 rounded-2xl mb-3' resizeMode='cover'/>}
            </View>
          </View>
        </View>
        {selectedPost.comments.map(comment=>(
          <View key={comment._id} className='border-b border-gray-100 bg-white p-4'>
            <View className='flex-row'>
              <Image source={{uri:comment.user.profilePicture}} className='szie-10 rounded-full mr-3'/>
              <View className='flex-1'>
                <View className='flex-row items-center mb-1'>
                  <Text className='font-bold text-gray-900 mr-1'>{comment.user.firstName} {comment.user.lastName}</Text>
          ...
          <View className='flex-row'>
            ...
            <View className='flex-1'>
              ...
              disabled={isCreatingComment || !commentText.trim()}>
                {isCreatingComment ? (<ActivityIndicator size={'small'} color={"white"}/>):(<Text className={`font-semibold ${commentText.trim() ? "text-white" :"text-gray-500"}`}>Reply</Text>)}
              </TouchableOpacity>
            </View>
          </View>
        </View>
